...
namespace ECS::Systems
{
    namespace
    {
        ...
        {
            if (value == "Migrate")
                return Database::MigrationMode::Migrate;

            if (value == "Validate")
                return Database::MigrationMode::Validate;

            if (value == "Disabled")
                return Database::MigrationMode::Disabled;
        ...
        Database::DBEntry ReadBundle(const nlohmann::json& config, std::string_view bundle)
        {
            ...
            Database::DBEntry entry(merged.at("address"), merged.at("port"), merged.at("database"), merged.at("user"), merged.at("password"),
                ParseMigrationMode(bundleConfig.value("migrationMode", "Migrate")), bundleConfig.value("createIfMissing", false));
    ...
    void DatabaseSetup::Init(entt::registry& registry)
    {
        ...
        nlohmann::ordered_json fallback = {
            { "Connection", { { "address", "127.0.0.1" }, { "port", 5432 }, { "database", "postgres" },
            ...
            { "Bundles", { { "Auth", { { "migrationMode", "Migrate" }, { "createIfMissing", false } } },
        ...
        if (!permissionSyncResult)
            throw std::runtime_error("Failed to synchronize the Auth permission catalogue (" + std::string(Database::OperationFailureName(permissionSyncResult.Failure())) + ")");

        auto permissionResult = gameCache.database->LoadPermissionTables();
        if (!permissionResult)
            throw std::runtime_error("Failed to load the Auth permission cache (" + std::string(Database::OperationFailureName(permissionResult.Failure())) + ")");
        gameCache.permissionTables = std::move(permissionResult).Value();

        auto worldResult = gameCache.database->LoadWorldCache();
        if (!worldResult)
            throw std::runtime_error("Failed to load the World database cache (" + std::string(Database::OperationFailureName(worldResult.Failure())) + ")");

        Database::WorldCacheSnapshot worldSnapshot = std::move(worldResult).Value();
        gameCache.factionRuntimeData = Gameplay::Faction::BuildRuntimeData(worldSnapshot.factions, worldSnapshot.creatures);
        if (!gameCache.factionRuntimeData)
            throw std::runtime_error("Failed to initialize faction runtime data due to invalid World content");
        ...
        gameCache.factionTables = std::move(worldSnapshot.factions);
    }
