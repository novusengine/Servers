...
{
    if (_registries.gameRegistry == nullptr)
        return;

    ECS::Singletons::NetworkState* networkState = _registries.gameRegistry->ctx().find<ECS::Singletons::NetworkState>();
    if (networkState == nullptr || networkState->server == nullptr)
        return;

    networkState->server->Stop();
    networkState->server.reset();
}
